...
async def help(ctx):
    ...
    embed = discord.Embed(color=0xff4800)
    embed.set_image(url='https://i.ibb.co/fGqd8FW/help1.jpg')
    await ctx.send(embed=embed)
    ...
    try:
        ...
        try:
            if json_data['fact'] in users.get(author.id).get('facts'):
                await ctx.send(
                    embed=discord.Embed(color=0xFF2B2B, title=f"Кажется этот факт вы уже слышали,"
                                                              f" сейчас подберем другой"))
        ...
        try:
            if json_data_2['image'] in users.get(author.id).get('pictures'):
                await ctx.send(
                    embed=discord.Embed(color=0xFF2B2B, title=f"Кажется это фото вы уже видели,"
                                                              f" сейчас подберем другое"))
                responce_2 = requests.get(f'https://some-random-api.ml/animal/{animal}')
                json_data_2 = json.loads(responce_2.text)
            else:
    ...
    try:
        ...
        try:
            if json_data['image'] in users.get(author.id).get('memes'):
                await ctx.send(
                    embed=discord.Embed(color=0xFF2B2B, title=f"Кажется этот мем вы уже видели,"
                                                              f" сейчас подберем другой"))
    ...
    try:
        ...
        try:
            if json_data['joke'] in users.get(author.id).get('jokes'):
                await ctx.send(
                    embed=discord.Embed(color=0xFF2B2B, title=f"Кажется эту шутку вы уже слышали,"
                                                              f" сейчас подберем другую"))
                responce = requests.get(f'https://some-random-api.ml/meme')
                json_data = json.loads(responce.text)
            else:
    ...
    try:
        ...
        try:
            if json_data['articles'][number]['title'] in users.get(author.id).get('news'):
                await ctx.send(
                    embed=discord.Embed(color=0xFF2B2B, title=f"Кажется эту новость вы уже слышали,"
                                                              f" сейчас подберем другую"))
